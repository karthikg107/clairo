...
const HIGH_CONFIDENCE_PAIRS = new Set([
  ...
  "hi-IN", "de-DE", "es-ES", "es-MX", "es-AR",
  "fr-FR", "pt-BR", "pt-PT", "ar-SA", "ar-AE",
  "ar-EG", "ur-PK", "zh-cn-CN", "zh-tw-TW",
  "ja-JP", "ko-KR", "it-IT", "nl-NL", "pl-PL",
  "ru-RU", "tr-TR",
]);
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <span aria-hidden className="text-dark-400 shrink-0">▾</span>
      </button>
            ...
            {filtered.map((opt) => (
              <li
                ...
                className={[
                  ...
                  opt.value === value ? "bg-brand-50 font-medium text-brand-700" : "text-dark-900",
                ].join(" ")}
              >
                <span>{opt.label}</span>
                {opt.sublabel && (
...
export function LanguageSelection({ onSubmit, initialValues = {} }: LanguageSelectionProps) {
  const t = useTranslations("language_selection");
  const formId = useId();
  ...
  return (
    ...
    >
      ...
      <p className="mt-3 text-xs text-center text-dark-400">
        {t("cta_hint")}
      </p>
    </form>
  );
