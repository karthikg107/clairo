    ...
    >
      {items.map(({ level, key }) => (
        <span key={level} role="listitem" className="flex items-center gap-1">
          <span
            className={[
              'inline-block w-3 h-3 rounded-sm border border-dark-200',
              LEVEL_CLASSES[level] || 'bg-dark-100',
            ].join(' ')}
            aria-hidden
          />
          {t(key)}
  ...
  return (
    <section aria-label={t('page_label', { n: page.page_number })}>
      <div className="flex items-center justify-between mb-2">
        ...
        {lowPct > 0 && (
          <span className="text-xs text-danger-700 font-medium" aria-live="polite">
      ...
      >
        {page.words.map((w, i) => (
          <React.Fragment key={i}>
            <WordToken word={w} onChange={(next) => onWordChange(i, next)} />{' '}
          </React.Fragment>
        ))}
        {page.words.length === 0 && (
...
export function OcrReview({
  ...
  const handleConfirm = () => {
    if (!confirmed) return
    // CLR-045 — user confirmed the reviewed text; analysis begins.
    // Metadata only: page count, never text.
    track('analysis_started', { page_count: pages.length })
    onConfirm(pages)
  }
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background" dir={dir}>
      ...
      >
        {/* Header */}
        <h1 id={headingId} className="text-xl font-bold text-dark-900 mb-1">
          {t('heading')}
        </h1>
        <p className="text-sm text-dark-600 mb-4">{t('subheading', { filename })}</p>
      ...
      >
        ...
        <label htmlFor={checkboxId} className="flex items-start gap-3 cursor-pointer">
          ...
          />
          <span className="text-sm text-dark-800 leading-snug">{t('confirm_label')}</span>
        </label>
